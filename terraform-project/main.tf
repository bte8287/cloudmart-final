...
#  bucket = "my-unique-bucket-${random_id.bucket_suffix.hex}"  # Creates a unique name using the random suffix
...
######UPDATED TERRAFORM CODE FOR BIG QUERY UPDATES TO LAMBDA FUNCTIONS AND SUCH######
...
resource "aws_lambda_function" "dynamodb_to_bigquery" {
  ...
  environment {
    variables = {
      GOOGLE_CLOUD_PROJECT_ID        = "cloudmart-455611"
